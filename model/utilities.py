...
def load_data(val_num=200, read_rgb=True, read_depth=False):
    __read_all(read_depth=read_depth, read_rgb=read_rgb)


def __read_all(read_rgb=True, read_depth=False):
    ...
    def __split_name(labels, name):
        ...
        label = np.array([v, r], dtype=np.float32)
        print(splitted)
        labels.append(label)
